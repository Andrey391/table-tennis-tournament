  ...
  return (
    <Layout>
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="min-w-0">
          ...
          <select value={city} onChange={e => setCity(e.target.value)}
            ...
            {cities.map(c => <option key={c.city} value={c.city} className="bg-[#101f36] text-white">{c.city}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-5">
        {[
          { to: "/players", label: "home.tiles.players", d: "M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8" },
          ...
          { to: "/tournament/new", label: "home.newTournament", d: "M12 5v14M5 12h14" },
        ].map(tile => (
      ...
      {tournaments.length === 0 ? (
        <div className="text-center py-14 bg-[#101f36] rounded-lg border border-[#1c3350] px-4">
          <p className="text-[#6b84a0] mb-4 text-sm">{t("home.empty")}</p>
        ...
        <div className="space-y-2">
          {tournaments.map(tr => (
            <Link key={tr.id} to={`/tournament/${tr.id}`}

...
export default function CreateTournament() {
  ...
  const [form, setForm] = React.useState({
    name: "", type: "SINGLE", system: "ROUND_ROBIN", format: "BEST_OF_3",
    tablesCount: 4, maxGroups: 1, playersPerGroup: 8, playersOut: 2,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const r = await api.tournaments.create(form);
      ...
      <div className="max-w-xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 bg-gray-800 p-6 rounded-lg">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1">Max Groups</label>
              <input type="number" min={1} value={form.maxGroups} onChange={e => set("maxGroups", +e.target.value)}
                className="w-full px-4 py-2 bg-gray-700 rounded border border-gray-600 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Players/Group</label>
              <input type="number" min={2} value={form.playersPerGroup} onChange={e => set("playersPerGroup", +e.target.value)}
                className="w-full px-4 py-2 bg-gray-700 rounded border border-gray-600 focus:outline-none" />
            </div>
          </div>

          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => navigate("/")} className="flex-1 bg-gray-600 text-white py-2 rounded hover:bg-gray-500">Cancel</button>
            <button type="submit" disabled={loading} className="flex-1 bg-blue-600 text-white py-2 rounded hover:bg-blue-700 disabled:opacity-50">

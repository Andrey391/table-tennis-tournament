...
function computeStandings(players: any[], matches: any[]) {
  ...
  for (const pl of players) {
    if (!pl.user) continue;
    stats.set(pl.userId, { userId: pl.userId, firstName: pl.user.firstName, lastName: pl.user.lastName, club: pl.user.club, rating: pl.user.rating, wins: 0, losses: 0, setsWon: 0, setsLost: 0, buchholz: 0, ratingChange: 0 });
  ...
  for (const m of matches) {
    ...
    // Net rating movement across the event: the winner gains eloDelta, the loser gives it up.
...
async function finishMatch(match: any, rated = true) {
  ...
  // Snapshot both ratings before Elo moves them (see "best win" in the statistics).
  const [r1, r2] = await Promise.all([match.player1Id, match.player2Id].map((id: string | null) =>
    id ? d.user.findUnique({ where: { id }, select: { rating: true } }) : null));
...
// completed matches (mirror of server/src/shared/stats.ts). Nothing here is cached or
// denormalised onto User: a reopened match simply drops out of the next read.

type StatPlayer = { id: string; firstName: string; lastName: string; rating: number };
...
function computePlayerStats(userId: string, currentRating: number, matches: StatMatch[], placings: number[]) {
...
function computeHeadToHead(a: string, matches: StatMatch[]) {
...
type LeaderMetric = "rating" | "wins" | "played";
...
function computeLeaders(matches: StatMatch[], metric: LeaderMetric, limit = 20) {
...
function periodStart(period: string | undefined, now = new Date()): Date | null {
...
app.get("/api/results", async (req, res: any) => {
  try {
...
app.get("/api/players/:id/stats", async (req, res: any) => {
  try {
    const userId = req.params.id;
    const user = await db().user.findUnique({ where: { id: userId }, select: { rating: true } });
    if (!user) { res.status(404).json({ error: "Not found" }); return; }
    const [matches, finished] = await Promise.all([
      db().match.findMany({ where: { status: "COMPLETED", OR: [{ player1Id: userId }, { player2Id: userId }] }, select: statMatchSelect }),
...
app.get("/api/players/:id/h2h/:otherId", async (req, res: any) => {
  try {
...
app.get("/api/leaders", async (req, res: any) => {
  try {

...
2. **Pairing / rounds** — `POST /tournaments/:id/pair` (manager only) generates the next round and flips the tournament to **ACTIVE**:
   ...
   - **Odd headcount**: whoever has played the *fewest* matches so far (rating breaks ties) sits out that round with no match and no stat change — this rotates the bye fairly across rounds instead of always landing on the same player.

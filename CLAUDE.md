...
Sets exist, but a fixed **best of N** does not — asking for one is a new feature (a `bestOf` column plus an auto-finish check in `/score`), not a restoration of removed code. The same goes for "bracket" and "group" features — the old `Group`/`Bracket`/`Team`/`Game` Prisma models, `BracketPage.tsx`, and the round-robin/olympic/double-elimination generators are gone.
...
**There is no fixed "best of N".** `/matches/:id/start` opens set 1. `/score` adds a point to the current set; when a set is won, its winner is credited to the match's tally and the *next set opens immediately* — finishing a set never finishes the match. The judge calls it with `/end`, and whoever won more sets takes the match. An equal tally is a draw and moves nobody's rating. `/undo` reverses one point in the current set, and if that set is empty because the previous one just ended, it steps back into the previous set, reopens it and takes back the set win. `/forfeit` records a single set at the target score so a walkover still leaves consistent per-set stats.

`Match.pointsToWin` (11 or 21, settable only while `NOT_STARTED`) is the target each of its sets inherits; every set also stores its own copy so historical stats stay accurate if it ever changes.

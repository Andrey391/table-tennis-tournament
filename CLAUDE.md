...
**Profile stats** (`GET /profile/stats`, `server/src/routes/profile.ts`, mirrored in `api/index.ts`): computes `tournaments` (count of `TournamentUser` rows with `status: REGISTERED`), `matches`/`wins`/`losses` (from `COMPLETED` matches where the caller is `player1` or `player2`) live from the database — never cached or denormalized onto `User`.
...
When asked to add or fix an API endpoint, mirror the change in **both** `server/src/routes/*.ts` and `api/index.ts`, and keep `server/prisma/migration.sql` in sync with `schema.prisma` if you change the schema. That file is a hand-maintained **full reset**, not a real Prisma migration and not an incremental one: it drops every application table and recreates them empty, so applying it always costs all existing data. Keep it that way — add new columns/tables to the `CREATE TABLE` statements rather than bolting `ALTER TABLE` sections onto the end. Creating the first account afterwards needs `npx tsx server/prisma/seed.ts`, because `server/`'s `POST /auth/register` requires an already-authenticated `ADMIN`/`ORGANIZER` and an empty database has neither. If unsure which target the user cares about, ask.

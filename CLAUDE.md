...
1. **DRAFT** — organizer creates a tournament (name + table count) and adds participants from the existing player roster (`POST /tournaments/:id/players`). Roster can be freely edited (add/remove) while in this state.
2. **Pairing** — `POST /tournaments/:id/pair` locks the roster, seeds everyone by `User.rating` (highest first), pairs adjacent ranks (1v2, 3v4, ...; an odd player out gets a bye and no match), assigns table numbers round-robin across `tablesCount`, and flips the tournament to **ACTIVE**. This is a one-shot operation — there is no re-pairing or additional rounds built in.
3. **ACTIVE** — each `Match` is scored independently. Before a match starts, the judge can set its point target to 11 or 21 (`PUT /matches/:id` with `pointsToWin`, only allowed while `NOT_STARTED`); this is a per-match choice, not a tournament-wide setting. Scoring, deuce, and server-rotation logic live in `server/src/shared/scoring.ts` (`isDeuce`, `getMatchWinner`, `nextServerSide`), duplicated inline in `api/index.ts` for the Vercel deployment.
4. Standings (`GET /tournaments/:id/standings`) are a flat win/loss/points-for/points-against table computed directly from completed matches — there are no groups.
...
`User` (login account, also the tournament participant — `Match.player1`/`player2` reference `User` directly, not a separate `Player` model), `Tournament`, `TournamentUser` (roster join table with `seed`), `Match` (now holds `pointsToWin`, `score1`/`score2`, `serverSide`, `letCount`, `lastScorer`/`prevServerSide` for undo, `tableNumber` — one match is one game to the target score, no nested `Game` rows), `AuditLog`, `Session`.
...
**Auth**: JWT (`jsonwebtoken`), bearer token in `Authorization` header, secret from `JWT_SECRET` env var. `server/src/middleware/auth.ts` exports `authMiddleware` (required auth) and `roleMiddleware(...roles)` (RBAC gate). Roles: `ADMIN`, `ORGANIZER`, `JUDGE`, `PLAYER`, `VIEWER`. Passwords hashed with bcryptjs. Registration through the UI (`Register.tsx`) requires an already-authenticated `ADMIN`/`ORGANIZER` (`POST /auth/register` is gated by `roleMiddleware`), so it's effectively an admin "add player to the roster" form, not public self-signup.
